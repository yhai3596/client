document.addEventListener('DOMContentLoaded', () => {
    ...
    // Global state to store the raw analysis data
    let currentAnalysisData = null;

    function exportMarkdown(data) {
        let mdContent = `# 朋友圈客户画像分析报告\n\n`;
        mdContent += `## 💡 核心洞察\n${data.coreInsight}\n\n`;
        ...
        const dimensionTitles = {
            ...
            psychology: "🧠 心理画像",
            consumption: "💰 消费分析",
            riskOpportunity: "⚖️ 风险机会",
            social: "👥 社交影响",
            strategy: "🎯 销售策略"
        };

        for (const [key, section] of Object.entries(data.dimensions)) {
            const title = dimensionTitles[key] || section.title;
            mdContent += `## ${title}\n`;
            
            section.items.forEach(item => {
                // Replace HTML <br> with newline for Markdown
                const value = item.value.replace(/<br>/g, '\n  > ');
                mdContent += `- **${item.label}**:\n  > ${value}\n`;
            });
            mdContent += `\n`;
        }

        mdContent += `---\nGenerated by Alan，更多AI应用请关注公众号：Alan的AI时代`;

        // Create download
        const blob = new Blob([mdContent], { type: 'text/markdown;charset=utf-8' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = '朋友圈客户画像分析报告.md';
    ...
    window.exportReport = async (format = 'pdf') => {
        // Special handling for Markdown export
        if (format === 'md') {
            if (!currentAnalysisData) {
                alert('请先生成报告后再导出。');
                return;
            }
            exportMarkdown(currentAnalysisData);
            return;
        }
